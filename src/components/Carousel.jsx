  ...
  return (
    <>
        ...
        <div className="bg-almost-white px-5 py-3.5 border-x border-b border-black/7 rounded-b-2xl flex items-center justify-between gap-4 min-h-[48px]">
          ...
          {items.length > 1 && (
            <div className="flex gap-1.5 flex-shrink-0">
              {items.map((_, i) => (
                <button key={i} onClick={() => setIdx(i)} className={`rounded-full transition-all duration-200 ${i === idx ? 'bg-bright-green w-4 h-1.5' : 'bg-black/20 w-1.5 h-1.5 hover:bg-black/40'}`} aria-label={`Aller à l'image ${i + 1}`}/>
              ))}
            </div>
          )}
        </div>
      ...
      {lightbox && (
        ...
        >
          <button onClick={() => setLightbox(false)} className="absolute top-[72px] right-5 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 transition-colors flex items-center justify-center text-white z-10" aria-label="Fermer le visualiseur d'images">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M18 6L6 18M6 6l12 12"/></svg>
          </button>

          {items.length > 1 && (
            <>
              <button onClick={prev} className="absolute left-4 top-1/2 w-11 h-11 rounded-full bg-white/10 hover:bg-white/20 transition-colors flex items-center justify-center text-white z-10" aria-label="Image précédente">
